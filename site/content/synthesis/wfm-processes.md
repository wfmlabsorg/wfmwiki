...
By expressing a range of outcomes, simulations can facilitate a change in how we introduce a new goal for the Future WFM model: {{< wikilink "wfm-goals-risk-ratings" "Risk Ratings." >}}
...
When it becomes necessary to allocate resources to other channels, the operating standard again suggests automation to continuously monitor the primary channel (for example voice: calls in queue, or voice: agents available) to trigger automations to reallocate to a secondary channel when conditions are met. This is similar automations listed {{< wikilink "wfm-processes-real-time-automation" "Real-Time Automation" >}}.
